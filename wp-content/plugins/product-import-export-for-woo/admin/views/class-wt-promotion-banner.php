...
if ( ! class_exists( 'Wt_Promotion_Banner' ) )  {
	class Wt_Promotion_Banner {
		...
		/**
         * wp-content/plugins/product-import-export-for-woo/assets
    	 * 	Add banner scripts and styles
    	 * 	
    	 */
		public function add_banner_scripts() {
			...
			if( $this->is_show_banner() ) { // Show the banner
                ...
                $src=   WT_P_IEW_PLUGIN_URL.'assets/images/banners/webtoffee_logo_transparent.svg';
                $image = WT_P_IEW_PLUGIN_URL.'assets/images/banners/webtoffee_logo.svg';
				?>
		        <style type="text/css">
		            .wt_promotion_banner{ background:#fff url('<?php echo esc_url($src)?>') right no-repeat; display:block; clear:both; box-sizing:border-box; }
		            .wt_promotion_banner_inner{ display:inline-block; width:100%; padding:20px 15px; box-sizing:border-box; }
		            .wt_promotion_banner_left{ float:left; width:100px; height:90px; background:#F9F9F9 url('<?php echo esc_url( $image)?>')  center no-repeat; border-radius:4px;}
