(function ($) {
    ...
    $(window).on('load', function () {
        ...
        var afrsm_popup = document.getElementById('afrsm-pro-popup');

        // Function to open the popup
        function afrsm_openPopup(selectedValue, selectedValuegroup) {
            $('.pro-feature-content' ).hide();
            afrsm_popup.style.display = 'flex';
            if( 'Attribute Specific' === selectedValuegroup ){
                afrsm_popup.querySelector('.attribute_in_pro-content').style.display = 'block';
            } else if( 'Cart Specific' === selectedValuegroup ){
                afrsm_popup.querySelector('.cart_specific_in_pro-content').style.display = 'block';
            } else if ( 'Checkout Specific' === selectedValuegroup ) {
                afrsm_popup.querySelector('.checkout_specific_in_pro-content').style.display = 'block';
            } else {
                afrsm_popup.querySelector('.' + selectedValue + '-content').style.display = 'block';
            }
        }

        // Function to close the popup
        function afrsm_closePopup() {
            afrsm_popup.style.display = 'none';
        }
        ...
        $('body').on('click', '#afrsm_premium_purchase', function(){
            let handler;
            afrsm_closePopup();
            handler = FS.Checkout.configure({
                plugin_id: '3379',
                image: 'https://www.thedotstore.com/wp-content/uploads/sites/45/2018/09/advanced_flat_rate_shipping_method-1.png?quality=90',
            });
            handler.open({
                name: 'Ad. Flat Rate Shipping For WooCommerce Pro',
        ...
        $('body').on('change', '.product_fees_conditions_condition', function (e) {
            ...
            var selectedValuegroup  = $('option:selected', this).parent().attr('label');
                                            
             if( selectedOption.hasClass('afrsm-pro') ){
                afrsm_openPopup(selectedValue, selectedValuegroup);
